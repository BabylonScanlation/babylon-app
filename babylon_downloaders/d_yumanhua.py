        ...
        for match_val in matches:
            ...
            for seed in seeds:
                try:
                    ...
                    final_bytes = base64.b64decode(xored + b"=="[:pad2])
                    final = final_bytes.decode("utf-8", errors="ignore")
                    ...
                    try:
                        ...
                        if isinstance(data_json, list):
                            urls = [str(u) for u in data_json if "http" in str(u)]
                            if urls: return urls
                    except:
                        pass
...
class YumanhuaLogic:
    def parse_series_page(self, slug: str) -> tuple[str, str, str, list[ChapterDict]]:
        ...
        # 2. Intentar obtener el resto vía AJAX y añadir al final
        try:
            ...
            if r2.status_code == 200:
                ...
                if str(data.get("code", "")) == "200" and "data" in data:
                    ...
                    for item in data_items:
                        cid = str(item.get("chapterid", ""))
                        cname = str(item.get("chaptername", ""))
...
# ══════════════════════════════════════════════════════════════
class DownloaderYumanhua(BaseDownloader):
    NAME = "YUMANHUA (yumanhua.com)"
    HAS_CATALOG = True
    HAS_SEARCH = True

    def __init__(self):
        _load_seeds()
        self.logic = YumanhuaLogic()

    def search(self, query: str) -> list[dict]:
        res = self.logic.search(query)
        for r in res:
            r["id"] = r["slug"]
        return res

    def get_catalog(self, **kwargs) -> list[dict]:
        # El catálogo completo es pesado, pero BaseDownloader lo espera
        res = load_full_catalog()
        for r in res:
            r["id"] = r["slug"]
        return res
    ...
    def get_series(self, item: dict) -> tuple[dict, list[dict]]:
        ...
        title, autor, sinopsis, chapters = self.logic.parse_series_page(slug)
        meta = {
            "id": slug,
            "slug": slug,
            "title": title,
    ...
    def get_chapter_images(self, chapter: dict, series: dict) -> list[str]:
        # Reconstruir el dict esperado por YumanhuaLogic
        cap_dict: ChapterDict = {
            "slug": chapter["slug"],
            "title": chapter["title"],
            "url": chapter["url"],
            "html": None
        }
        return self.logic.extract_images(cap_dict)

    def dl_image(self, url: str, referer: str = "") -> Optional[bytes]:
        try:
            r = SESSION.get(url, timeout=(5, 15), headers={"Referer": referer or BASE_URL + "/"})
            if r.status_code == 200:
                return r.content
        except Exception:
            pass
