...
if TYPE_CHECKING:
    ...
    from options_menu import ImageOptions, MiscOptions, OptionsMenu, VideoOptions
...
class OptionsController(QObject):
    ...
    def save_gemini_settings(
        ...
        try:
            ...
            QMessageBox.critical(
                self.app, "Error", f"No se pudo guardar la configuración de Gemini: {e}"
            )
    ...
    def pause_resume_video(self):
        """Pausa o reanuda la reproducción del video. (Video deshabilitado)"""
        pass
    ...
    def load_local_video(self):
        """Cargar video local para fondo. (Video deshabilitado)"""
        pass

    def load_video_from_url(self, url: str):
        """Cargar video desde URL externa con barra de progreso. (Video deshabilitado)"""
        pass

    def start_download_thread(self, url: str):
        """Inicia el hilo de descarga del video. (Video deshabilitado)"""
        pass
    ...
    def update_video_source(self, source: str):
        """Actualizar la fuente del video y liberar recursos previos. (Video deshabilitado)"""
        pass
    ...
    def load_local_image(self):
        ...
        if misc_page.bg_type_combo.currentText() != "Imagen":
            image_page = cast(
                "ImageOptions", self.app.options_menu.options_pages["image"]
            )
    ...
    def show_video_warning(self, widget: QWidget):
        ...
        warning_label.setStyleSheet(
            "color: gray; font-size: 12px; background-color: transparent;"
        )
